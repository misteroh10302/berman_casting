...
class RouterIndex extends Component {
  ...
  render() {
    ...
    const allMessages = this.state.contents.map(function(reg,i) {
        return <Route key={i} path={"/"+ reg.fields.title} exact render={()=><Post postClassName={reg.fields.tag} relatedContent={theRelatedContent} titleContent={theRelatedContent} contactMobile={theContactContent}  postContent={reg}/>}/>;
      }
  )
              ...
              <div key={location.pathname}>
                ...
                <Switch location={location}>
                  <Route path="/" exact className="index" render={()=><App num="2" contactMobile={this.state.contactContent} titleContent={this.state.contents} gridContents={this.state.contents}/>}/>
                    {allMessages}
                  <Route path="/faces" className="faces"  exact render={()=><GridwithSideBar num="2" contactMobile={this.state.contactContent}     contactMobile={this.state.contactContent} titleContent={this.state.contents} gridContents={this.state.facesContent}/>}/>
                  <Route path="/commercial" className="commercial" exact render={()=><GridwithSideBar num="2" contactMobile={this.state.contactContent}  contactMobile={this.state.contactContent} titleContent={this.state.contents} gridContents={this.state.commercialContent}/>}/>
                  ...
                  <Route path="/contact" className="contact" exact render={()=><Contact contactMobile={this.state.contactContent} titleContent={this.state.contents} gridContents={this.state.contactContent}/>}/>
